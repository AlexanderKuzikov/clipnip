...
| Дата | Изменение |
|------|-----------|
| 2026-08-04 | v0.2.1: переносимость — проверка WebView2 Runtime (реестр) + MessageBox, файловый лог `%LOCALAPPDATA%\clipnip\clipnip.log`, логирование ошибок info/download, самовосстановление бинарников (ensureBins перед каждым запуском yt-dlp), recover в воркерах; UI: индитерминант-бар «Connecting to server…» + elapsed, уменьшены отступы (без прокрутки), «Save to» под полем ссылок, title страницы |
