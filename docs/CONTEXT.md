...
| Дата | Изменение |
|------|-----------|
| 2026-08-04 | v0.4.0: пауза/продолжение per-item (kill + докачка через .part) и общая (pauseall/resumeall, acquire ждёт); отмена удаляет недокачанное (cancelall); старт параллельности 8 (потолок 10); фикс parseSpeed (срезался неверный суффикс — скорость всегда была 0); маркированные ошибки «killed: paused» (нет гонки pause↔requeue); UI: кнопки Pause/Resume/Cancel на карточках, Pause All/Resume All/Cancel All, общий прогресс-бар и проценты в счётчике, скорость в карточке |
