...
| Адаптивная параллельность | работает | старт 5 → потолок 20; +1/10 успешных; ÷2 при сбое; cooldown 429 = 30 с |
| Авторетраи | работает | 3×5 с, stall-детект 20 с, socket-timeout 15, фатальные не ретраятся |
...
| Дата | Изменение |
|------|-----------|
| 2026-08-04 | v0.3.0: адаптивная параллельность (семафор-пермиты, старт 5/потолок 20/пол 1, +1 за 10 успешных, ÷2 при сбое, cooldown 30 с при 429), авторетраи 3×5 с + классификация ошибок (fatal/429/network), stall-детект 20 с, очередь 1024 + неблокирующий enqueue, плейлисты до 500, socket-timeout 15, UI-счётчик «Completed N of M». Проверено: 30 джобов на живом соединении (0 отказов, рост 5→8), юнит-тесты адаптации/классификации |
